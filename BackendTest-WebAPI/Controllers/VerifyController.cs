...
namespace BackendTestWebAPI.Controllers
{
    ...
    public class VerifyController : BaseController
    {
        ...
        {
            var verification = _context.Verifications //get latest verification code
                .OrderByDescending(a => a.Id)
                .Where(a => a.Username == username && a.Timestamp >= DateTime.Today && a.ExpirationTime >= DateTime.Now)
                .AsNoTracking()
                .FirstOrDefault();
